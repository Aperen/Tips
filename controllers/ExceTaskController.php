...
if(strtotime($ret['exce_time'])-strtotime(date("Y-m-d h:i:sa"))<300){
    ...
    if(strtotime($ret['exce_time'])-strtotime(date("Y-m-d h:i:sa"))>0) {
        ...
        $mailcontent = "小主，您有最新的提醒，快来看一看哦：<br>{$ret['content']}";//邮件内容
        ...
        $log=new Log();				//日志类实例化
        $log->insertLog(date("Y-m-d h:i:sa"),"ExceTaskcController","邮件发送状态:".$state);
        var_dump($ret);
    }else{
        echo "执行了第二个分支";
    }
